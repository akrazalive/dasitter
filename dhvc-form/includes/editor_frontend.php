...
class DHVCForm_Editor_Frontend extends Vc_Frontend_Editor
{
    ...
    public static function getInlineUrl($url = '', $id = '')
    {
        ...
        return apply_filters('dhvc_form_get_inline_url', admin_url() . 'edit.php?dhvc_form_editor=frontend&post_id=' . $the_ID . '&post_type=' . get_post_type($the_ID) . (strlen($url) > 0 ? '&url=' . rawurlencode($url) : ''));
    }

    function render($template)
    {
        if ('editor' === $template)
            dhvc_form_include_editor_template('editor_frontend.tpl.php', array(
                'editor' => $this
            ));
        else
            vc_include_template('editors/frontend_' . $template . '.tpl.php', array(
                'editor' => $this
            ));
    }
}