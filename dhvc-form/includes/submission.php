...
class DHVCForm_Submission{
	...
	private function _setup_posted_data(){
		...
		foreach ((array)$fields as $field){
			...
			if(!in_array($field['tag'], dhvc_form_ignore_fields())){
				$entries_data[$name] = $value;
			}
		}
		$this->_entries_data = apply_filters( 'dhvc_form_entries_data', $entries_data, $this );
	...
	private function _verify_nonce($form_id){
		if(!dhvc_form_submit_use_nonce($form_id)){
			return true;
		}
