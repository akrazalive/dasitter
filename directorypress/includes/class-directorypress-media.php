...
class directorypress_media_handler {
	...
	public function upload_media_image() {
		...
		if (wp_verify_nonce($_POST['_wpnonce'], 'upload_clogo') && isset($_POST['browse_file_clogo']) && isset($_POST['post_id']) && ($post_id = $_POST['post_id'])) {
			if (($listing = directorypress_get_listing($post_id))) {
				if (directorypress_user_permission_to_edit_listing($post_id)) {
					// insert attachment ID to the post meta
					update_post_meta($post_id, '_attached_image_clogo', $_POST['browse_file_clogo']);
					echo json_encode($_POST['browse_file_clogo']);
				}
			}
		}
		if (wp_verify_nonce($_POST['_wpnonce'], 'upload_images') && isset($_POST['browse_file_cover_image']) && isset($_POST['post_id']) && ($post_id = $_POST['post_id'])) {
			if (($listing = directorypress_get_listing($post_id))) {
				if (directorypress_user_permission_to_edit_listing($post_id)) {
					// insert attachment ID to the post meta
					update_post_meta($post_id, '_attached_image_cover', $_POST['browse_file_cover_image']);
