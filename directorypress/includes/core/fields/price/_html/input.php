	...
	<div class="row">
		...
		<?php if($field->has_frontend_currency): ?>
			...
			<div class="col-md-3 col-sm-6 col-xs-12">
				...
				<input type="text" name="directorypress-field-price-frontend-currency-<?php echo $field->id; ?>" class="directorypress-field-input-currency form-control" placeholder="<?php echo _e('Currency Symbol', 'DIRECTORYPRESS'); ?>" value="<?php // echo esc_attr($field->data['frontend_currency']); 
                if(isset($field->data['frontend_currency']))
                                                {
                                                	echo esc_attr($field->data['frontend_currency']); 
                                                }
                                                else
                                                {
                                                	echo "";
		...
		<div class="col-md-3 col-sm-6 col-xs-12">
			...
			<input type="text" name="directorypress-field-input-<?php echo $field->id; ?>" class="directorypress-field-input-price form-control" placeholder="<?php echo _e('Insert Price', 'DIRECTORYPRESS'); ?>" value="<?php // echo esc_attr($field->data['value']);
                                                if(isset($field->data['value']))
                                                {
                                                	echo esc_attr($field->data['value']); 
                                                }
                                                else
                                                {
                                                	echo "";
